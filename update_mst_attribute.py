...
# GraphQL query to fetch category data
query = """
query GetMstAttributes {
  mst_attributes {
    isdeleted
		options
		name
...
def fetch_data_from_hasura():
    ...
    if response.status_code == 200:
        return response.json().get('data', {}).get('mst_attributes', [])
    else:
...
def update_json_file(filename='mst_attribute.json'):
