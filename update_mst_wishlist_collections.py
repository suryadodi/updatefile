...
query = """
query GetMstWishlistCollections {
  mst_wishlist_collections {
    id
		user_id
		name
		shop_id
...
def fetch_data_from_hasura():
    ...
    if response.status_code == 200:
        return response.json().get('data', {}).get('mst_wishlist_collections', [])
    else:
...
def update_json_file(filename='mst_wishlist_collections.json'):
